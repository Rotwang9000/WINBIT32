...
function App() {
  ...
  const [ desinationAmt, setDesinationAmt ] = useState('3');
...
async function getAQuote(sellAmt = 0, loop = 0) {
  ...
  if (desinationAmt !== "MAX" && isNaN(desinationAmt.replace("%", ""))) return;
  ...
  if (
    sellAmt === 0 &&
    (desinationAmt === "MAX" || // or contains a % sign
      desinationAmt.substring(desinationAmt.length - 1) === "%") &&
    wallets[0] &&
    wallets[0].balance[0] &&
    wallets[0].balance[0].assetAmount > 0
  ) {
    var bal = parseFloat(wallets[0].balance[0].assetAmount.toString());
    var pct = parseFloat(desinationAmt.replace("%", ""));
  ...
  if(!isNaN(desinationAmt)){
    eoms_pc = parseFloat(routes[0].expectedOutputMaxSlippage.toString()) / desinationAmt;
    //get abs difference
  ...
  if(loop > 5){ //if we've tried 5 times, give up
    setDesinationAmt(routes[0].expectedOutputMaxSlippage.toString());
    setSellAmount(0);
  ...
  if (sellAmt === 0 || eoms_ok === false) {
    ...
    var destAmt = parseFloat(desinationAmt);
    if (desinationAmt === "MAX") {
          ...
          try {
            switch (walletOption) {
              case WalletOption.KEYSTORE: {
                ...
                return skClient.connectKeystore(connectChains, ph.trim()).catch((error) => {  
                 setError(error);
                  return null;
...
useEffect(() => {
  const interval = setInterval(() => {
    ...
    if 
    (originBalances && originBalances.length > 1 &&
     wallets[0] && 
      wallets[1] &&
       wallets[0].balance[0] && wallets[1].balance[0] && 
       (wallets[0].balance[0].assetAmount > originBalances[0] || wallets[1].balance[0].assetAmount > originBalances[1]) ) {
      getAQuote();
    }
...
useEffect(() => {
  ...
  if(desinationAmt !== 'MAX' && document.getElementById('desination_amt').dataset.oAutoSwap === '' && !isNaN(desinationAmt) && desinationAmt !== sellAmount){
    getAQuote();
  }
...
useEffect(() => {
  ...
  if(wallets[0] && wallets[1] && wallets[0].balance[0] && wallets[1].balance[0]){
...
//update balances
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    genConnect();
    //set desination address from query string
    ...
    if(dest_addr){
      ...
      document.getElementsByClassName('input_destination_addr')[0].style.display = 'none';
      //show fixed_destination_addr
      document.getElementsByClassName('fixed_destination_addr')[0].style.display = 'block';
    }else{
      //show input_destination_addr
      document.getElementsByClassName('input_destination_addr')[0].style.display = 'block';
      //hide fixed_destination_addr
      document.getElementsByClassName('fixed_destination_addr')[0].style.display = 'none';
    }
    //set desination amount from query string
    const dest_amt = urlParams.get('amt');
    if(dest_amt){
      setDesinationAmt(dest_amt);
      //hide input_destination_amt
      document.getElementsByClassName('input_destination_amt')[0].style.display = 'none';
      //show fixed_destination_amt
      document.getElementsByClassName('fixed_destination_amt')[0].style.display = 'block';
    }else{
      //show input_destination_amt
      document.getElementsByClassName('input_destination_amt')[0].style.display = 'block';
      //hide fixed_destination_amt
      document.getElementsByClassName('fixed_destination_amt')[0].style.display = 'none';
...
function doSwap() {
  ...
  useEffect(() => {
    ...
    if(destinationAddr.substring(0,2) === '0x'){
      setTransferType(["btc", "eth"]);
    }else if (destinationAddr.substring(0,1) === '1' || destinationAddr.substring(0,3) === 'bc1'){
    ...
    <div style={styles.container}>
      ...
      <div className="hflex_whenwide">
        ...
        <div>
          ...
          ></textarea>
          <div id="error_phrase" className={msgColour}>{msg}</div>
          <div><input type="checkbox" id="autoswap" name="autoswap" checked={autoswap} onChange={(e) => setAutoswap(e.target.checked)} /><label htmlFor="autoswap">Auto Swap</label>
          </div>
        </div>
      </div>
      <hr />
      <div className="hflex_whenwide">
        ...
        <div className="input_destination_amt">
          ...
          <input
            ...
            onBlur={(e) => {setAutoswap(e.target.dataset.oAutoSwap === "true"); e.target.dataset.oAutoSwap = ''; getAQuote();}}
            onChange={(e) => setDesinationAmt(e.target.value)}
            value={desinationAmt}
            data-o-auto-swap=""
          />
        </div>
        <div className="fixed_destination_amt">{desinationAmt}</div>
      </div>
      <div>
      ...
      <br />
      <div className="input_slippage"> Received amount could be 1% different due to slippage. Swap Fee: 1%</div>

      <button type="button" onClick={() => { console.log(skClient.connectedChains); doSwap();}}>
        Swap
      </button>
